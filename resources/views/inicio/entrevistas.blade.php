...
<head>
    ...
    <title>Entrevistas - El Pionero de Valparaíso</title>
    ...
    <div class="w-full mx-auto p-4">
        ...
        <div>
            ...
            <main class="w-full space-y-6 bg-gray-50 border border-gray-300 rounded-lg p-4 shadow-sm">
                <div>
                    <h2 class="text-2xl font-semibold text-gray-800 border-b pb-2 mb-6">Entrevistas</h2>

                    @if ($entrevistas->isNotEmpty())
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            @foreach ($entrevistas as $entrevista)
                                <a href="{{ url('entrevista/' . $entrevista->slug) }}"
                                    ...
                                    <div class="p-4">
                                        ...
                                        @if ($entrevista->entrevistado)
                                            <div class="text-sm text-[#fc5648] font-semibold mb-2">
                                                Entrevista a: {{ $entrevista->entrevistado }}
                                            </div>
                                        @endif
                                        <div class="text-sm text-gray-600 line-clamp-3">
                                            {{ Str::limit(strip_tags($entrevista->contenido), 150) }}
                                        </div>
                                        @if ($entrevista->fecha_publicacion)
                                            <div class="text-xs text-gray-500 mt-3">
                                                {{ \Carbon\Carbon::parse($entrevista->fecha_publicacion)->format('d M Y') }}
                                            </div>
                                        @endif
                                    </div>
                                </a>
                    ...
                    @else
                        <p class="text-gray-600 text-center py-8">No hay entrevistas disponibles.</p>
                    @endif
                </div>
            </main>
