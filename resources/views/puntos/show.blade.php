...
@section('content')
    <div class="w-full mx-auto">
        ...
        <main
            ...
            x-init="registrarEvento('{{ $punto->slug }}', 'visita')"
        ...
        >
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <aside class="hidden lg:flex lg:col-span-2 flex-col gap-3 pt-2">
                    ...
                    @if($punto->lat && $punto->lng)
                    <button @click="$dispatch('geo-modal'); registrarEvento('{{ $punto->slug }}', 'como_llegar')"
                    ...
                    <a href="https://wa.me/{{ preg_replace('/\D/', '', $punto->whatsapp_publico) }}"
                       target="_blank" rel="noopener"
                       @click="registrarEvento('{{ $punto->slug }}', 'whatsapp')"
                       class="w-full py-3 px-4 rounded-2xl text-sm font-bold text-left transition-all duration-200 flex items-center gap-2 bg-white text-gray-600 border border-gray-200 hover:border-gray-400">
                ...
                <div class="lg:col-span-6">
                    ...
                    <div x-show="vista === 'carta'" x-cloak
                        ...
                        <div class="mb-6 flex items-center justify-between">
                            ...
                            @endif
                            @if($datoCarta['url'] ?? null)
                                <a href="{{ $datoCarta['url'] }}"
                                   target="_blank" rel="noopener"
                                   class="inline-flex items-center gap-2 px-4 py-2 bg-gray-900 text-white text-sm font-bold rounded-xl hover:bg-pindoor-accent transition">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
                            ...
                            @endif
                            @if(!$textoCartaLimpio && ($datoCarta['pdf_ruta'] ?? null))
                            ...
                            @endif
                            @if(!$textoCartaLimpio && ($datoCarta['url'] ?? null))
                                <div class="flex flex-col items-center justify-center gap-4 py-6 text-center">
                                    <svg class="w-12 h-12 text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/>
                                    </svg>
                                    <p class="text-sm text-gray-500">La carta está disponible en un enlace externo.</p>
                                    <a href="{{ $datoCarta['url'] }}"
                                       target="_blank" rel="noopener"
                                       class="inline-flex items-center gap-2 px-6 py-3 bg-[#fc5648] text-white text-sm font-bold rounded-xl hover:bg-[#d94439] transition">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="lg:bg-white lg:rounded-[2.5rem] lg:p-8 lg:shadow-xl lg:shadow-gray-200/50 lg:border lg:border-white lg:sticky lg:top-8">
                        ...
                        @if($punto->lat && $punto->lng)
                            <div class="hidden lg:block">
                                ...
                                <button onclick="window.dispatchEvent(new CustomEvent('geo-modal')); registrarEvento('{{ $punto->slug }}', 'como_llegar')"
    ...
    <a href="https://wa.me/{{ preg_replace('/\D/', '', $punto->whatsapp_publico) }}"
       target="_blank" rel="noopener"
       onclick="registrarEvento('{{ $punto->slug }}', 'whatsapp')"
       class="lg:hidden fixed z-50 flex items-center justify-center w-12 h-12 bg-[#25D366] text-white rounded-full shadow-xl transition"
       style="right: 16px; bottom: calc(150px + var(--inset-bottom, 0px))"
