...
class TraducirCategorias extends Command
{
    protected $signature   = 'categorias:traducir {--id= : ID de la categoría específica} {--force : Sobreescribir traducciones existentes} {--to=en : Idioma destino (en|fr)}';
    protected $description = 'Traduce categorías de ES a EN/FR usando MyMemory API';
    ...
    public function handle(): int
    {
        ...
        foreach ($categorias as $categoria) {
            $nombreEs      = $categoria->getTranslation('nombre', 'es', false);
            $descripcionEs = $categoria->getTranslation('descripcion', 'es', false);

            $nombreDestino      = $categoria->getTranslation('nombre', $to, false);
            $descripcionDestino = $categoria->getTranslation('descripcion', $to, false);

            $force = $this->option('force');

            $this->info("Categoría #{$categoria->id}: {$nombreEs}");
            ...
            $categoria->setTranslation('nombre',      $to, $nombreDestino)
